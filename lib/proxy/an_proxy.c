...
an_destroy (ANCONN conn)
{
	...
	if (conn->proxy_domain)
		free (conn->proxy_domain);

	if (conn->proxy_hostname)
		free (conn->proxy_hostname);
	if (conn->hostname)
		free (conn->hostname);
	if (conn->authbuf)
...
int
an_set_credentials_with_domain (ANCONN conn, const char *user, const char *pass, const char *domain)
{
	int retval = 0;
	retval = an_set_credentials(conn, user, pass);
	if (retval != AN_ERROR_SUCCESS)
		return retval;
	if (conn == NULL)
		return AN_ERROR_INVALIDARG;

	if (conn->proxy_domain != NULL)
		free (conn->proxy_domain);
	
	if (domain == NULL) {
		conn->proxy_domain = NULL;
	} else {
		conn->proxy_domain = (char *) malloc (strlen (domain) + 1);
		if (conn->proxy_domain == NULL)
			return AN_ERROR_NOMEM;
		strcpy (conn->proxy_domain, domain);
	}
...
int an_test_proxy(int proxyType,
...
{
	int retval = 0;
	ANCONN an_proxy = an_new_connection();
	if ( proxyType == 4 )
	{
		char proxy_server[1024];
		char tmp[64]={0};
		sprintf (tmp, "%i", proxyPort);
		strcpy(proxy_server, "socks5://");// 需求中只要求支持socks5
		strcat(proxy_server, proxyServer);
		strcat(proxy_server, ":");
		strcat(proxy_server, tmp);
		retval = an_set_proxy_url(an_proxy, proxy_server);
		if (retval != AN_ERROR_SUCCESS)
			return retval;
		if (useProxyAuth)
		{
			retval = an_set_credentials_with_domain(an_proxy, proxyUsername, proxyPassword, proxyDomain);	
			if (retval != AN_ERROR_SUCCESS)
				return retval;
		}
	}
	...
	{
		// Destiny IP:Port port should be 443 for most proxy server
		//close SOCKSPROXY
		char proxy_server[1024];
		char tmp[64]={0};
		sprintf (tmp, "%i", proxyPort);
		strcpy(proxy_server, "https://");
		strcat(proxy_server, proxyServer);
		strcat(proxy_server, ":");
		strcat(proxy_server, tmp);
		retval = an_set_proxy_url(an_proxy, proxy_server);
	...
	{
		// something wrong here. no valid proxyinfo here.
		return AN_ERROR_PROXY;
	}

	retval = an_connect_tohostname(an_proxy, hostname, port);
