...
struct auth_s *new_auth(void) {
	...
	tmp = (struct auth_s *)mynew(sizeof(struct auth_s));
	tmp->user = mynew(MINIBUF_SIZE);
	tmp->domain = mynew(MINIBUF_SIZE);
	tmp->workstation = mynew(MINIBUF_SIZE);
	tmp->passntlm2 = 0;
	tmp->passnt = 0;
	tmp->passlm = 0;
	tmp->hashntlm2 = 1;
