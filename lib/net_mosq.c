...
int _mosquitto_packet_write(struct mosquitto *mosq)
{
	...
	while(mosq->current_out_packet){
		...
		while(packet->to_process > 0){
			if (mosq->proxy_)
				write_length = _mosquitto_proxy_write(mosq, &(packet->payload[packet->pos]), packet->to_process);
			else
...
#endif
{
	...
	if(!mosq->in_packet.command){
		if (mosq->proxy_)
			read_length = _mosquitto_proxy_read(mosq, &byte, 1);
		else
...
#  endif
	...
	while(mosq->in_packet.to_process>0){
		if (mosq->proxy_)
			read_length = _mosquitto_proxy_read(mosq, &(mosq->in_packet.payload[mosq->in_packet.pos]), mosq->in_packet.to_process);
		else
